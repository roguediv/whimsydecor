...
import { CloseInfoForm, resetInfoForm, StartInfoForm, StartInputCheck, UpdateInfoForm } from '@/components/scripts/client/popup/InfoHandler';
...
type props = {
  ...
  icon?: string;
  deleteId?: string;
  inputCheck?: {title: string, desc: string, btn1: string, btn2: string} | null;
  Trigger?: () => Promise<void>;
  RedirectTrigger?: (test: number) => Promise<ReturnField>;
}

const Button = forwardRef<HTMLTextAreaElement, props>(({className = "", text = "", name="", icon="", deleteId = '', inputCheck = null, Trigger = () => {}, RedirectTrigger = (test: number) => {if (test == 0) return {status: 2, title: '', desc: '', data: '0'}}}, ref) => {
  /// Reusable front-end button that handles different back-end functions
  /// * Runs database functions
  /// * Handles redirects from server
  /// * Can be referenced by a parent component using useRef
  ...
  async function onClick() {
    ...
    if (inputCheck) {
      StartInputCheck({title: inputCheck?.title, desc: inputCheck.desc, btn1: inputCheck?.btn1, btn2: inputCheck.btn2})
      const userResponse = await awaitUserInput();
      resetInfoForm();
      if (userResponse === 0) return endClientQuery('', '');
    }
    const isTrigger = await RedirectTrigger(0);
    if (isTrigger && isTrigger.data === '1') { // Check if the trigger returns a redirect link
      StartInfoForm({title: "Loading...", desc: ""}, false)
      const url = await RedirectTrigger(1);
      if (!url) return endClientQuery("That Didn't Work", "Please try again later.");
      if (!url.data) return endClientQuery(url.title, url.desc);
      prismaExecutionService.endQuery();
      await router.push(url.data);
      CloseInfoForm();
      return;
    }
    if (deleteId != '') document.getElementById(deleteId)?.remove()
    await Trigger(); 
    prismaExecutionService.endQuery();
  }
  ...
  return (
    <button className={`${className}`} aria-label="Button" onClick={onClick}>
      {icon != '' ? 
      (() => {
        switch (icon) {
          case 'FiPlusCircle':
          ...
          case 'exit':
            return<><RiLogoutCircleLine/>{text}</>
          default:
            return <>{text}</>;
        }
      })()
